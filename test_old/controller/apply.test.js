...
describe('/applies', () => {
    ...
    it('POST|PUT|DELETE|GET /applies', async () => {
        await app.factory.createMany('users', 4);
        ...
        let data = await app
            .httpRequest()
            .get(
                `/api/v0/applies?objectType=${objectType}&objectId=${objectId}&applyType=0`
            )
            .expect(res => assert(res.statusCode == 200))
            .then(res => res.body);
        ...
        data = await app
            .httpRequest()
            .get(
                `/api/v0/members?objectType=${objectType}&objectId=${objectId}`
            )
            .set('Authorization', `Bearer ${token}`)
            .expect(res => assert(res.statusCode == 200))
            .then(res => res.body);
        ...
        data = await app
            .httpRequest()
            .get(
                `/api/v0/applies/state?objectType=${objectType}&objectId=${objectId}&applyId=3&applyType=0`
            )
            .set('Authorization', `Bearer ${token}`)
            .expect(res => assert(res.statusCode == 200))
            .then(res => res.body);
