...
describe('管理员接口', () => {
    ...
    it('001 管理员接口', async () => {
        ...
        const userToken = await app
            .httpRequest()
            .get('/api/v0/admins/userToken?userId=2')
        ...
        const list1 = await app
            .httpRequest()
            .get('/api/v0/admins/query?sql=select * from users where id > 0')
            .set('Authorization', `Bearer ${token}`)
            .expect(res => assert(res.statusCode == 200))
            .then(res => res.body);
        ...
        await app
            .httpRequest()
            .put('/api/v0/admins/users/' + resource.id)
            .send({ sex: 'M' })
            .set('Authorization', `Bearer ${token}`)
            .expect(res => assert(res.statusCode == 200))
            .then(res => res.body);
        ...
        resource = await app
            .httpRequest()
            .get('/api/v0/admins/users/' + resource.id)
            .set('Authorization', `Bearer ${token}`)
            .expect(res => assert(res.statusCode == 200))
            .then(res => res.body);
        ...
        await app
            .httpRequest()
            .delete('/api/v0/admins/users/' + resource.id)
            .set('Authorization', `Bearer ${token}`)
            .expect(res => assert(res.statusCode == 200))
            .then(res => res.body);
