...
describe('/users', () => {
    ...
    it('0001 修改密码 token失效测试', async () => {
        ...
        // 构建用户
        const user = await app.factory
            .create('users', { username: 'user0000', password: md5('123456') })
            .then(o => o.toJSON());

        // 登录
        const token1 = await app
            .httpRequest()
            .post(`${apiUrlPrefix}users/login`)
            .send({ username: 'user0000', password: '123456' })
        ...
        const profile = await app
            .httpRequest()
            .get(`${apiUrlPrefix}users/profile`)
        ...
        await app
            .httpRequest()
            .put(`${apiUrlPrefix}users/pwd`)
            .send({ password: '123456', oldpassword: '123456' })
    ...
    it('0002 注册 登录 获取指定用户 用户profile token信息获取 用户信息更新 用户基本信息更新 用户活跃度 用户详情 用户余额', async () => {
        let user = await app
            ...
            .send({ username: 'xiaoyao', password: '123456' })
            .expect(res => assert(res.statusCode == 200))
            .then(res => res.body);
        assert(user);
        ...
        await app
            .httpRequest()
            .post(`/api/v0/users/${user.id}/contributions`)
            .send({ count: 2 })
            .set('Authorization', `Bearer ${token}`)
            .expect(res => assert(res.statusCode == 200))
            .then(res => res.body);
    ...
    it('0003 手机验证', async () => {
        // 构建用户
        await app.factory
            .create('users', { username: 'user0001', password: md5('123456') })
            .then(o => o.toJSON());
        //await app.model.users.create({username:"user0001", password: md5("123456")}).then(o => o.toJSON());
        const token = await app
            .httpRequest()
            .post(`/api/v0/users/login`)
            .send({ username: 'user0001', password: '123456' })
            .expect(res => assert(res.statusCode == 200))
            .then(res => res.body.token);
        assert(token);
        ...
        let data = await app
            .httpRequest()
            .get('/api/v0/users/cellphone_captcha?cellphone=' + cellphone)
            .expect(res => assert(res.statusCode == 200));
        ...
        await app
            ...
            .send({ cellphone, captcha: cache.captcha, isBind: true })
            .set('Authorization', `Bearer ${token}`)
            .expect(res => assert(res.statusCode == 200));
        ...
        let ok = await app
            .httpRequest()
            .post('/api/v0/users/cellphone_captcha')
            .send({ cellphone, captcha: cache.captcha, isBind: false })
            .set('Authorization', `Bearer ${token}`)
            .expect(res => assert(res.statusCode == 200))
            .then(res => res.body);
        assert(!ok);
    ...
    it('0004 邮箱验证', async () => {
        ...
        await app
            .httpRequest()
            .get('/api/v0/users/email_captcha?email=' + email)
            .expect(res => assert(res.statusCode == 200));
        ...
        let ok = await app
            ...
            .send({ email, captcha: cache.captcha, isBind: true })
            .set('Authorization', `Bearer ${token}`)
            .expect(res => assert(res.statusCode == 200))
            .then(res => res.body);
        assert(ok);
        ...
        ok = await app
            .httpRequest()
            .post('/api/v0/users/email_captcha')
            .send({ email, captcha: cache.captcha, isBind: false })
            .set('Authorization', `Bearer ${token}`)
            .expect(res => assert(res.statusCode == 200))
            .then(res => res.body);
        assert(!ok);
