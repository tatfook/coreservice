...
class Qiniu extends Service {
            ...
            operateManager.pfop(
                ...
                function(err, respBody, respInfo) {
                    ...
                    if (respInfo.statusCode === 200) {
                        ...
                        self.app.model.caches.put(persistentId, {
                            ...cache,
                            srcBucket: saveBucket,
                            srcKey: key,
                            dstBucket: saveBucket,
                            dstKey: 'thumb-' + key,
                        });
                        resolve(true);
                    } else {
    ...
    getUploadToken({ key, bucket }) {
        const { publicBucketName, mac, apiUrlPrefix } = this.getConfig();
        ...
        if (bucket === publicBucketName) {
            ...
            options.callbackBody =
                ...
                '{"key":"$(key)","hash":"$(etag)","size":$(fsize),"bucket":"$(bucket)","mimeType":"$(mimeType)","filename":"$(x:filename)","siteId":$(x:siteId)}';
    ...
    async upload({ key, content, bucket }) {
        ...
        const putPolicy = new qiniu.rs.PutPolicy({
            scope: (bucket || publicBucketName) + ':' + key,
        });
        const token = putPolicy.uploadToken(mac);
        ...
        const result = await new Promise((resolve, reject) => {
            ...
            formUploader.put(token, key, content, putExtra, function(
                respErr,
                respBody,
                respInfo
            ) {
    ...
    async delete({ key, bucket }) {
        ...
        const result = await new Promise(resolve => {
            bucketManager.delete(bucket || publicBucketName, key, function(
                respErr,
                respBody,
                respInfo
            ) {
    ...
    async move({ srcKey, dstKey, srcBucket, dstBucket }) {
        ...
        const result = await new Promise(resolve => {
            bucketManager.move(
                srcBucket || publicBucketName,
                srcKey,
                dstBucket || publicBucketName,
                dstKey,
                { force: true },
    ...
    async batchMove(list) {
        const { publicBucketName } = this.getConfig();
        const moveOperations = [];
        for (let i = 0; i < list.length; i++) {
            moveOperations.push(
                qiniu.rs.moveOp(
                    list[i].srcBucket || publicBucketName,
                    list[i].srcKey,
                    list[i].dstBucket || publicBucketName,
                    list[i].dstKey
                )
            );
        }
    ...
    async batchDelete(list) {
        ...
        _.each(list, item =>
            deleteOps.push(
                qiniu.rs.deleteOp(item.bucket || publicBucketName, item.key)
            )
        );
