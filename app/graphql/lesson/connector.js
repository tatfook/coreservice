...
class LessonConnector {
	constructor(ctx) {
		...
		this.packageLessonCountLoader = new DataLoader(async ids => {
			const list = await ctx.lessonModel.query(`select count(*) count, packageId id from packageLessons where packageId in (:ids) group by packageId`, {
				type: ctx.lessonModel.QueryTypes.SELECT,
				replacements:{
					ids,
				}
			});
			return ids.map(id => (_.find(list, o => o.id == id) || {count:0}).count);
		});

		this.packageTagLoader = new DataLoader(async ids => {
			const list = await ctx.model.tags.findAll({
				include: [
				{
					as: "systemTags",
			...
			return _.map(ids, id => {
				return list.filter(o => o.objectId == id).map(o => o.systemTags);
			});
		});
