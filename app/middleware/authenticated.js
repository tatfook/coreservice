...
module.exports = (options, app) => {
    ...
    return async function(ctx, next) {
        if (config.debug) {
            ...
            ctx.state.token = app.util.jwt_encode(
                ctx.state.user,
                config.secret
            );
            await next();
        ...
        const token =
            getCookieToken(ctx) ||
            getAuthorizationHeaderToken(ctx) ||
            ctx.query.token ||
            '';
        ctx.state.token = token;
        try {
            ctx.state.user = token
                ? app.util.jwt_decode(token, config.secret, false)
                : {};
            if (config.env !== 'unittest' && config.env !== 'local') {
                const isValid = await ctx.service.user.validateToken(
        ...
        try {
            ctx.state.admin = token
                ? app.util.jwt_decode(token, config.adminSecret, false)
                : {};
