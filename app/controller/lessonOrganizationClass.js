...
const LessonOrganizationClass = class extends Controller {
    ...
    async history() {
        ...
        if (!(roleId & CLASS_MEMBER_ROLE_ADMIN)) {
            return this.throw(400, '无权限');
        }

        const curtime = new Date();
        const count = await this.model.lessonOrganizationClasses.count({
    ...
    async index() {
        ...
        const sql =
            'select classId from lessonOrganizationClassMembers where organizationId = :organizationId and roleId & :roleId and memberId = :memberId';
        const ids = await this.ctx.model
            .query(sql, {
    ...
    async create() {
        ...
        const cls = await this.model.lessonOrganizationClasses
            .create(params)
            .then(o => o && o.toJSON());
        ...
        this.model.lessonOrganizationLogs.classLog({
            organizationId,
            cls,
            params,
            action: 'createClass',
    ...
    async update() {
        ...
        if (new Date(cls.end).getTime() < new Date().getTime()) {
            const organ = await this.model.lessonOrganizations
                .findOne({ where: { id: cls.organizationId } })
                .then(o => o && o.toJSON());
            await this.model.lessonOrganizationClasses.update(params, {
                where: { id: params.id },
            });
            const studentCount = await this.model.lessonOrganizations.getStudentCount(
                cls.organizationId
            );
            if (studentCount > organ.count) {
                // 还原修改
                await this.model.lessonOrganizationClasses.update(cls, {
                    where: { id: params.id },
                });
        ...
        if (params.packages) {
            ...
            await this.model.lessonOrganizationPackages.destroy({
                where: { classId: params.id, organizationId },
            });
            await this.model.lessonOrganizationPackages.bulkCreate(datas);
        }
    ...
    async destroy() {
        ...
        await this.model.lessonOrganizationClasses.destroy({
            where: { id, organizationId },
        });
        await this.model.lessonOrganizationPackages.destroy({
            where: { classId: id, organizationId },
        });
    ...
    async latestProject() {
        ...
        const projects = await this.model.projects
            .findAll({
