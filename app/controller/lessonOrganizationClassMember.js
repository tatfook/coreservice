...
const LessonOrganizationClassMember = class extends Controller {
    ...
    async teacher() {
        ...
        const members = await this.model.lessonOrganizations.getTeachers(
            organizationId,
            classId
        );
        ...
        const list = await this.model.lessonOrganizationClassMembers
            .findAll({
                include: [
                    {
                        as: 'users',
    ...
    async student() {
        ...
        const { classId } = this.validate({ classId: 'number_optional' });
        const members = await this.model.lessonOrganizations.getMembers(
            organizationId,
            1,
            classId
        );
    ...
    async bulkCreate() {
        ...
        // // const ok = await this.model.lessonOrganizationClassMembers.findOne(
        //    {where:{organizationId: organizationId, memberId: {$in: memberIds}, roleId:{$ne: params.roleId}}});
        // // if (ok)	return this.throw(400, "存在其它身份");

        // if (!(roleId & CLASS_MEMBER_ROLE_ADMIN)) {
        //     if (roleId <= CLASS_MEMBER_ROLE_STUDENT) {
        //         return this.throw(411, '无权限');
        //     }
        ...
        // });
        // const result = await this.model.lessonOrganizationClassMembers.bulkCreate(
        //     datas
        // );
    ...
    async create() {
        ...
        if (!params.memberId) {
            ...
            const user = await this.model.users
                .findOne({ where: { username: params.memberName } })
                .then(o => o && o.toJSON());
        ...
        // const ok = await this.model.lessonOrganizationClassMembers.findOne(
        //     { where: { organizationId, memberId: params.memberId, roleId: { $ne: params.roleId } } });
        // if (ok)	return this.throw(400, "存在其它身份");
        ...
        // const curtime = new Date();
        let oldmembers = await this.model.lessonOrganizationClassMembers
            .findAll({
                order: [[ 'id', 'desc' ]],
                include: [
                    {
        ...
        ids.length &&
            (await this.model.query(
                `update lessonOrganizationClassMembers set roleId = roleId & ~${params.roleId} where id in (:ids)`,
        ...
        const members = await this.model.lessonOrganizationClassMembers.bulkCreate(
            datas
        );
        if (params.realname) {
            await this.model.lessonOrganizationClassMembers.update(
    ...
    async destroy() {
        ...
        const member = await this.model.lessonOrganizationClassMembers
            .findOne({ where: { organizationId, id } })
            .then(o => o && o.toJSON());
