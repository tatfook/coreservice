...
class Keepwork extends Controller {
    async email() {
        ...
        const ok = await this.ctx.service.email.send({
            html,
            to,
            from,
            subject,
    ...
    async postSvgCaptcha() {
        const { key, captcha } = this.validate({
            key: 'string',
            captcha: 'string',
        });
    ...
    async postPageVisit() {
        ...
        const timestamp = moment(moment(new Date()).format('YYYY-MM-DD'))
            .add(1, 'days')
            .unix();
    ...
    async statistics() {
        ...
        const paracraftCount = await app.model.projects.count({
            where: { type: PROJECT_TYPE_PARACRAFT },
        });
        const siteCount = await app.model.projects.count({
            where: { type: PROJECT_TYPE_SITE },
        });
        const projectCount = paracraftCount + siteCount;

        const sql =
            'select count(*) count from projects where privilege & :recuritValue';
