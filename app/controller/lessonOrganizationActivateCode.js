...
const LessonOrganizationActivateCode = class extends Controller {
    ...
    async create() {
        ...
        const organ = await this.model.lessonOrganizations.getValidOrganizationById(
            organizationId
        );
        ...
        const list = await this.model.lessonOrganizationActivateCodes.bulkCreate(
            datas
        );
    ...
    async activate() {
        ...
        let { key, realname, organizationId } = this.validate({
            key: 'string',
            organizationId: 'number_optional',
        });

        const curtime = new Date().getTime();
        const data = await this.model.lessonOrganizationActivateCodes
            .findOne({ where: { key, state: 0 } })
            .then(o => o && o.toJSON());
        if (!data) return this.fail({ code: 2, message: '无效激活码' });

        if (organizationId && data.organizationId !== organizationId) {
            return this.fail({ code: 7, message: '激活码不属于这个机构' });
        }
        organizationId = data.organizationId;

        const cls = await this.model.lessonOrganizationClasses
            .findOne({ where: { id: data.classId } })
            .then(o => o && o.toJSON());
        ...
        if (isClassStudent) {
            return this.fail({ code: 6, message: '已经是该班级学生' });
        }
        ...
        if (!isStudent) {
            ...
            if (organ.count <= usedCount) {
                return this.fail({ code: 5, message: '人数已达上限' });
            }
        }

        let m = _.find(ms, o => o.classId === data.classId);
        const roleId = m
            ? m.roleId | CLASS_MEMBER_ROLE_STUDENT
            : CLASS_MEMBER_ROLE_STUDENT;
        if (m) {
            ...
            m = await this.model.lessonOrganizationClassMembers
                .create({
                    organizationId: data.organizationId,
