...
class Migration extends Controller {
	generateAttrsText(attrs) {
		...
		for (let key in attrs) {
			...
			const defaultValue = _.isString(value.defaultValue) ? `"${value.defaultValue}"` : value.defaultValue;
			text += `
			"${key}": {
				type: ${type},
				allowNull: ${value.allowNull},
				primaryKey: ${value.primaryKey},
				autoIncrement: ${key == "id" ? true : false},
	...
	async generateText({tableName, model}) {
		const indexes = await model.query(`show index from ${tableName}`, {type: model.QueryTypes.SHOWINDEXES}).then(list => list.map(o => ({
			primary:o.primary,
			fields: o.fields,
			unique: o.unique,
			name:o.name,
		})));
		const attrs = await model.query(`describe ${tableName}`, {type: model.QueryTypes.DESCRIBE});
...
module.exports = {
	...
	async generateAll() {
		...
		const keepworkTables = await this.app.model.query(`show tables`, {type: this.app.model.QueryTypes.SHOWTABLES});
		...
		const lessonTables = await this.app.lessonModel.query(`show tables`, {type: this.app.lessonModel.QueryTypes.SHOWTABLES});
	...
	async generate() {
		const datetime = moment().format("YYYYMMDDHHmmss");
		const {tableName, schema="keepwork"} = this.validate({tableName:"string"});
		const model = schema == "keepwork" ? this.app.model : this.app.lessonModel;
