...
const ProxyUser = class extends Controller {
    formatUserInfo(olduser, newuser) {
        ...
        if (realname) {
            olduser.realNameInfo = { cellphone: realname, verified: true };
        }
    }
    ...
    async register() {
        ...
        if (words.length) {
            return this.success({
                error: { id: -1, message: '包含铭感词', data: words },
            });
        }
        if (!usernameReg.test(username)) {
            return this.success({ error: { id: -1, message: '用户名不合法' } });
        }
        let user = await this.model.users.getByName(username);
        if (user) {
            return this.success({ error: { id: -1, message: '用户已存在' } });
        }
        ...
        if (!ok) this.logger.warn('创建git用户失败');
    ...
    async profile() {
        ...
        if (!user) {
            return this.success({ error: { id: -1, message: '用户不存在' } });
        }
    ...
    async changepw() {
        const { userId } = this.authenticated();
        const config = this.app.config.self;
        const { oldpassword, newpassword } = this.validate({
            oldpassword: 'string',
            newpassword: 'string',
        });
        const data = await axios
            .post(
                config.keepworkBaseURL + 'user/changepw',
        ...
        if (!data || data.error.id !== 0) {
            return this.success(
                data || { error: { id: -1, message: '修改失败' } }
            );
        }

        await this.model.users.update(
            { password: this.app.util.md5(newpassword) },
            { where: { id: userId } }
        );
    ...
    async batchChangePwd() {
        ...
        const token = this.app.util.jwt_encode(
            { roleId: 10, username: 'xiaoyao', userId: 3 },
            config.secret
        );
        ...
        for (let i = 0; i < list.length; i++) {
            ...
            let result = await axios
                .post(
                    config.keepworkBaseURL + 'tabledb/query',
                    {
                        tableName: 'user',
                        page: 1,
                        pageSize: 10,
            ...
            if (
                ...
                md5password !== oldUser.password
            ) {
                item.password = '密码错误';
                continue;
            }
            const newpassword = this.app.util.md5(
                item.newpassword || item.password
            );
            result = await axios
                .post(
                    config.keepworkBaseURL + 'tabledb/upsert',
                    {
                        tableName: 'user',
                        query: {
                            _id: oldUser._id,
