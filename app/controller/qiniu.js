...
const Qiniu = class extends Controller {
    ...
    async uploadToken() {
        ...
        const mac = new qiniu.auth.digest.Mac(
            config.qiniuPublic.accessKey,
            config.qiniuPublic.secretKey
        );
        const putPolicy = new qiniu.rs.PutPolicy(options);
        const token = putPolicy.uploadToken(mac);
    ...
    async fop() {
        ...
        await this.ctx.service.qiniu.persistentHandle({ key, bucket, fop });
    }
    ...
    async test() {
        this.ctx.service.qiniu.persistentHandle({
            key: 'abc.png',
            fop: 'imageView2/3/w/380/h270',
        });
