...
const ProxyOauthApp = class extends Controller {
    ...
    async getTokenByCode() {
        const { client_id, code, username } = this.validate({
            client_id: 'string',
            code: 'string',
        ...
        if (!cache) {
            return this.success(
                { error: 10001, message: 'authorized code invalid' },
                503
            );
        }
        if (username && cache.username !== username) {
            this.success({ error: 10003, message: 'username error' }, 503);
        }

        const user = await this.model.users.getById(cache.userId);
        if (!user) {
            return this.success(
                { error: 10002, message: 'server inner errror' },
                503
            );
        }
