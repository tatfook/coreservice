...
const LessonOrganizationUser = class extends Controller {
    ...
    async batchCreateUser() {
        ...
        const userinfos = users.map(u => ({
            userId: u.id,
            registerUsername: _.toString(u.id + 10000),
        }));
    ...
    async unbind() {
        ...
        const members = await this.model.lessonOrganizationClassMembers
            .findAll({ where: { classId, organizationId, bind: 1 } })
            .then(list => list.map(o => o.toJSON()));
    ...
    async setpwd() {
        ...
        const member = await this.model.lessonOrganizationClassMembers
            .findOne({ where: { organizationId, classId, memberId } })
            .then(o => o && o.toJSON());
        if (!member || member.bind === 0) {
            this.throw(400, '成员不存在或未绑定本机构');
        }
