...
module.exports = app => {
    ...
    storage.delete = async function(key) {
        ...
        const result = await new Promise(resolve => {
            bucketManager.delete(bucketName, key, function(
                respErr,
                respBody,
                respInfo
            ) {
    ...
    storage.batchDelete = async function(list) {
        ...
        _.each(list, item =>
            deleteOps.push(qiniu.rs.deleteOp(bucketName, item.key))
        );
    ...
    storage.list = async function(prefix = '', limit = 200, marker) {
        ...
        const bucketManager = getBucketManager();
        const result = await new Promise((resolve, reject) => {
            bucketManager.listPrefix(bucketName, options, function(
                respErr,
                respBody,
                respInfo
            ) {
    ...
    storage.imageAudit = async function(key) {
        ...
        const imgUrl =
            key.indexOf('http://') === 0 || key.indexOf('https://') === 0
                ? key
                : this.getDownloadUrl(key);
        const data = { data: { uri: imgUrl } };
        const signed = qiniu.util.generateAccessTokenV2(
            mac,
            uri,
            'POST',
            'application/json',
    ...
    storage.videoAudit = async function(id, key = '', async = true) {
        ...
        const videoUrl =
            key.indexOf('http://') === 0 || key.indexOf('https://') === 0
                ? key
                : this.getDownloadUrl(key);
        // const videoUrl = "http://oy41jt2uj.bkt.clouddn.com/97eb5420-708a-11e8-aaf9-f9dea1bb2117.mp4?e=2129423642&
        // token=LYZsjH0681n9sWZqCM4E2KmU6DsJOE7CAM4O3eJq:cny8ZH-tZl4PPMp_sUAn-chowHc=";
        const data = {
            data: { uri: videoUrl },
            params: {
                async,
