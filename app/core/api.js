...
class Api {
    ...
    async favorites(favorite) {
        if (favorite.objectType === ENTITY_TYPE_USER) {
            const fansUser = await this.app.model.users.getById(
                favorite.objectId
            );
            const followUser = await this.app.model.users.getById(
                favorite.userId
            );
    ...
    async usersUpsert(inst) {
        ...
        inst.projectCount = await this.app.model.projects.count({
            where: { userId },
        });
        inst.fansCount = await this.app.model.favorites.count({
            where: { objectId: userId, objectType: ENTITY_TYPE_USER },
        });
        inst.followCount = await this.app.model.favorites.count({
            where: { userId, objectType: ENTITY_TYPE_USER },
        });
        return this.curl(
            'post',
...
module.exports = app => {
    ...
    config.adminToken = app.util.jwt_encode(
