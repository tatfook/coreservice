...
describe('/users', () => {
    ...
    it('拉取用户消息', async () => {
        ...
        await app.model.userMessages.create({
            userId: 2,
            messageId: 6,
            state: 0,
        });
        ...
        await app
            .httpRequest()
            .post('/api/v0/userMessages/state')
            .send({ ids: [1, 2, 3], state: 1 })
            .set('Authorization', `Bearer ${token}`)
            .expect(res => assert(res.statusCode == 200))
            .then(res => res.body);
        ...
        data = await app
            .httpRequest()
            .get('/api/v0/userMessages?state=0')
            .set('Authorization', `Bearer ${token}`)
            .expect(res => assert(res.statusCode == 200))
            .then(res => res.body);
        assert(data.count == 2);
        ...
        await app
            .httpRequest()
            .post('/api/v0/userMessages/allstate')
            .set('Authorization', `Bearer ${token}`)
            .expect(res => assert(res.statusCode == 200))
            .then(res => res.body);
