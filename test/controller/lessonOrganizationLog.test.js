...
describe("机构", () => {
	...
	it("001 机构日志", async() => {
		...
		// 创建机构
		const organ = await app.model.lessonOrganizations.create({name:"org0000", count:1, endDate: "2119-01-01"}).then(o => o.toJSON());
		...
		// 允许老师管理学生
		await app.httpRequest().put("/api/v0/lessonOrganizations/" + organ.id).send({privilege: 1}).set("Authorization", `Bearer ${token}`).expect(200).then(res => res.body).catch(e => console.log(e));

		// 不允许老师管理学生
		await app.httpRequest().put("/api/v0/lessonOrganizations/" + organ.id).send({privilege: 2}).set("Authorization", `Bearer ${token}`).expect(200).then(res => res.body).catch(e => console.log(e));
	
		// 创建班级
		const cls = await app.httpRequest().post("/api/v0/lessonOrganizationClasses").send({organizationId: organ.id, name:"class000", begin: new Date(), end: new Date().getTime() + 1000 * 60 * 60 * 24}).set("Authorization", `Bearer ${token}`).expect(200).then(res => res.body).catch(e => console.log(e));
		assert(cls.id);
		...
		// 老师改名
		await app.httpRequest().post("/api/v0/lessonOrganizationClassMembers").send({organizationId: organ.id, classIds:[0], memberId:2, realname:"xiaoyao1", roleId: 2}).set("Authorization", `Bearer ${token}`).expect(200).then(res => res.body).catch(e => console.log(e));
	
		// 移除老师
		await app.httpRequest().post("/api/v0/lessonOrganizationClassMembers").send({organizationId: organ.id, classIds:[], memberId:2, realname:"xiaoyao", roleId: 2}).set("Authorization", `Bearer ${token}`).expect(200).then(res => res.body).catch(e => console.log(e));

		// 测试生成激活码
		await app.httpRequest().post("/api/v0/lessonOrganizationActivateCodes").send({organizationId: organ.id, count: 20, classId: cls.id}).set("Authorization", `Bearer ${token}`).expect(200).then(res => res.body.data).catch(e => console.log(e));

		// 添加班级老师
		member = await app.httpRequest().post("/api/v0/lessonOrganizationClassMembers").send({organizationId: organ.id, classIds:[0, cls.id], memberId:2, realname:"xiaoyao", roleId: 2}).set("Authorization", `Bearer ${token}`).expect(200).then(res => res.body).catch(e => console.log(e));
		// 移除班级老师
		member = await app.httpRequest().post("/api/v0/lessonOrganizationClassMembers").send({organizationId: organ.id, classIds:[0], memberId:2, realname:"xiaoyao", roleId: 2}).set("Authorization", `Bearer ${token}`).expect(200).then(res => res.body).catch(e => console.log(e));
