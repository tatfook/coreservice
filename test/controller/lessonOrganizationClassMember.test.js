...
describe("机构学生", () => {
	...
	it("001 机构学生添加接口测试", async() => {
		...
		// 测试接口添加学生
		await app.httpRequest().post("/api/v0/lessonOrganizationClassMembers").send({memberId: user.id, organizationId: organ.id}).set("Authorization", `Bearer ${token}`).expect(200).then(res => res.body.data).catch(e => console.log(e));
	});
});
