...
describe('oauth app', () => {
    ...
    it('002 认证码认证', async () => {
        ...
        const code = await app
            .httpRequest()
            .get('/api/v0/oauth_apps/oauth_code?client_id=123456')
            .set('Authorization', `Bearer ${token}`)
            .expect(res => assert(res.statusCode))
            .then(res => res.body.code);
        assert(code);

        // 获取token
        const auth_token = await app
            .httpRequest()
            .post('/api/v0/oauth_apps/oauth_token')
            .send({ client_id: '123456', client_secret: 'abcdef', code: code })
            .set('Authorization', `Bearer ${token}`)
            .expect(res => assert(res.statusCode))
