...
describe("lesson organization class", () => {
	...
	it("002 获取机构学生", async() => {
		...
		const user = await app.model.users.create({username:"user001", password: md5("123456")}).then(o => o.toJSON());
	...
	it("003 机构过期测试", async() => {
		...
		// 生成激活码
		await app.httpRequest().post("/api/v0/lessonOrganizationActivateCodes").set("Authorization", `Bearer ${token}`).send({classId:cls1.id, count:2}).expect(400).then(res => res.body);

		// 不过期机构
		await app.model.lessonOrganizations.update({endDate:"2119-01-01"}, {where:{id:organ.id}});
		const list = await app.httpRequest().post("/api/v0/lessonOrganizationActivateCodes").set("Authorization", `Bearer ${token}`).send({classId:cls1.id, count:2}).expect(200).then(res => res.body);
