...
describe('lesson organization class', () => {
  ...
  it('001 班级结业与恢复', async () => {
    ...
    // 登录机构
    const token = await app.httpRequest().post('/api/v0/lessonOrganizations/login').send({ organizationId: 1, username: 'user001', password: '123456' })
      .expect(200)
      .then(res => res.body.token)
      .catch(e => console.log(e));

    // 结业班级
    await app.httpRequest().put('/api/v0/lessonOrganizationClasses/' + cls.id).send({ end: '2019-01-01' })
    ...
    // 恢复结业班级
    await app.httpRequest().put('/api/v0/lessonOrganizationClasses/' + cls.id).send({ end: '2040-01-01' })
      .set('Authorization', `Bearer ${token}`)
      .expect(res => {
  ...
  it('002 获取机构学生', async () => {
    ...
    // 登录机构
    const token = await app.httpRequest().post('/api/v0/lessonOrganizations/login').send({ organizationId: organ.id, username: 'userxiaoyao', password: '123456' })
      .expect(200)
      .then(res => res.body.token)
      .catch(e => console.log(e));

    // 获取学生
    let students = await app.httpRequest().get('/api/v0/lessonOrganizationClassMembers/student').set('Authorization', `Bearer ${token}`)
      .expect(res => assert(res.statusCode === 200))
      .then(res => res.body);
    assert(students.count === 2);

    students = await app.httpRequest().get('/api/v0/lessonOrganizationClassMembers/student?classId=' + cls1.id).set('Authorization', `Bearer ${token}`)
      .expect(res => assert(res.statusCode === 200))
      .then(res => res.body);
    assert(students.count === 1);

    const teachers = await app.httpRequest().get('/api/v0/lessonOrganizationClassMembers/teacher').set('Authorization', `Bearer ${token}`)
      .expect(res => assert(res.statusCode === 200))
      .then(res => res.body);
  ...
  it('003 机构过期测试', async () => {
    ...
    // 生成激活码
    await app.httpRequest().post('/api/v0/lessonOrganizationActivateCodes').set('Authorization', `Bearer ${token}`)
      .send({ classId: 1, count: 2 })
      .expect(400)
      .then(res => res.body);

    // 不过期机构
    await app.model.lessonOrganizations.update({ endDate: '2119-01-01' }, { where: { id: 1 } });
    const list = await app.httpRequest().post('/api/v0/lessonOrganizationActivateCodes').set('Authorization', `Bearer ${token}`)
      .send({ classId: 1, count: 2 })
      .expect(200)
      .then(res => res.body);
    ...
    // 激活激活码
    const ok = await app.httpRequest().post('/api/v0/lessonOrganizationActivateCodes/activate').set('Authorization', `Bearer ${usertoken}`)
