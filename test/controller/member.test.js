...
describe('成员', () => {
    ...
    it('001 成员增删改查 批量创建成员 成员是否存在', async () => {
        ...
        await app
            .httpRequest()
            .delete(`/api/v0/members/${member2.id}`)
            .set('Authorization', `Bearer ${token}`)
            .expect(res => assert(res.statusCode == 200))
            .then(res => res.body);
        members = await app
        ...
        let exist = await app
            ...
            .send({ objectId, objectType, memberId: 2 })
            .set('Authorization', `Bearer ${token}`)
            .expect(res => assert(res.statusCode == 200))
            .then(res => res.body);
        assert(exist);
        ...
        exist = await app
            .httpRequest()
            .get(`/api/v0/members/exist`)
            .send({ objectId, objectType, memberId: 12 })
            .set('Authorization', `Bearer ${token}`)
            .expect(res => assert(res.statusCode == 200))
            .then(res => res.body);
        assert(!exist);
