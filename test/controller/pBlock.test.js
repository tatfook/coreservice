...
describe('paracraft 元件', () => {
    it('001 元件增删改查 使用', async () => {
        ...
        await app
            .httpRequest()
            .delete('/api/v0/pBlocks/' + block2.id)
            .set('Authorization', `Bearer ${token}`)
            .expect(res => assert(res.statusCode == 200))
            .then(res => res.body);
        // 列表
        ...
        await app
            .httpRequest()
            .post('/api/v0/pBlocks/' + blockId + '/use')
            .set('Authorization', `Bearer ${token}`)
            .expect(res => assert(res.statusCode == 200))
            .then(res => res.body);
        block = await app
    ...
    it('002 元件分类', async () => {
        // 测试系统元件获取
        const classify1 = await app.model.pClassifies.create({
            name: 'classify1',
        });
        const classify2 = await app.model.pClassifies.create({
            name: 'classify2',
        });
        const classify3 = await app.model.pClassifies.create({
            name: 'classify3',
            parentId: classify1.id,
        });
        ...
        app.model.pBlockClassifies.create({
            classifyId: classify1.id,
            blockId: block2.id,
        });
        app.model.pBlockClassifies.create({
            classifyId: classify3.id,
            blockId: block1.id,
        });
        app.model.pBlockClassifies.create({
    ...
    it('003 管理员接口', async () => {
        ...
        await app
            .httpRequest()
            .post('/api/v0/admins/pBlocks')
            .send({ userId: 1, pClassifies: [{ id: 1 }] })
            .set('Authorization', `Bearer ${token}`)
            .expect(res => assert(res.statusCode == 200))
            .then(res => res.body);
